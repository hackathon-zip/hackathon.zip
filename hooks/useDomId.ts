...
export function useDomId (identifier?: string) {
    const id = Array.from(Array(10).keys()).map(() => useId()).join("");

    let slugified = identifier?.toLowerCase().replace(/(^-|-$)/g, "").replace(/[^a-z0-9]+/g, "-").split('-').join(' ').trim().split(' ').join('-');
    if (slugified?.length && slugified.length > 0) slugified += "-";

    return slugified + md5(slugified + id).substring(0, 10);
}