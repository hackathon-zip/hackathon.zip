...
function Required () {
  return (
    <span style={{ color: 'red', scale: 0.8, left: '2px', position: 'relative' }}>*</span>
  )
}

function InputTuple ({ disabled, getValue, updateValue, label, mb, inputs, required }: { disabled?: boolean, getValue: any, updateValue: any, label?: string, mb?: number, inputs: { label: string, name: string, placeholder?: string, type?: string }[], required: boolean }): any {
  return (
    <>
      <style>{`
...
export function Form({ schema, submission }: { schema: FormSchema, submission: FormSubmission<any> }): any {
  const inputFields = Object.fromEntries(schema.elements.map((element: FormElement) => {
    if (element.type == 'tuple') {
      return element.inputs.map((input: FormTextInput) => input);
    } else if (element.name) {
      return element;
    }
  }).filter(a => a).flat().map((input) => [input?.name, input]) as [string, FormTextInput][]);

  const [values, setValues] = useState<any>(Object.fromEntries(Object.keys(inputFields).map<[string, FormValue]>(name => [name, { value: '', isValid: false, showWarning: false }])));
  ...
  const invalidFields = Object.keys(values).filter(name => !values[name].isValid);

  return (
    <>
      <form {...(submission.type == 'request' ? {
        ...
        {schema.elements.map((formElement: FormElement) => {
          if ('date text email password number'.split(' ').includes(formElement.type)) {
            ...
            return (
              <InputTuple disabled={loading} getValue={getValue} updateValue={updateValue} label={element.label} required={!!element.required} inputs={element.inputs.map((input: FormTextInput) => {
                return {
        ...
        <div style={{
          ...
          <Button loading={loading} type={invalidFields.length ? "default" : "success"} htmlType="submit" onClick={e => {
          ...
          {Object.values(values).filter((value: any) => value.showWarning).length > 0 && (
            <Text margin={0}>{invalidFields.length} invalid field{invalidFields.length > 1 && 's'}</Text>
          )}
        </div>
      </form>
    </>
