...
import { Breadcrumbs, Button, Popover, Select, Spinner, Tabs, Text } from "@geist-ui/core"
...
export default function Navbar ({ breadcrumbs }: { breadcrumbs: any }) {
    ...
    const { hackathon: activeHackathonSlug, feature } = useUrlState(['hackathon', 'feature']);
    const activeHackathon = hackathons.find((h: Hackathon) => h.slug === activeHackathonSlug);
    ...
    return (
            ...
            <div style={{
                ...
                <Breadcrumbs.Item>Page</Breadcrumbs.Item> */}
                    ...
                    <Popover content={<>
                        ...
                        {hackathons.filter((h: Hackathon) => h.slug !== activeHackathonSlug).map((h: Hackathon) => (
                            <Popover.Item>
        ...
        <div style={{
            width: '100%',
                    ...
                    <Tabs onChange={value => {
                        router.push(`/${activeHackathonSlug}/${value}`)
                    }} defaultValue={feature}>
                        <Tabs.Tab label="Register" value="register" />
                        <Tabs.Tab label="Check-In" value="check-in" />
                        <Tabs.Tab label="Broadcast" value="broadcast" />
                        <Tabs.Tab label="Schedule" value="schedule" />
                        <Tabs.Tab label="Ship" value="ship" />
                        <Tabs.Tab label="Integrate" value="integrate" />
                    </Tabs>
                </div>
      </div>
    )
